    ...
    with console.status("[dim]LLM thinking...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                ...
                "structured PRAnalysis with a summary, risk_factors, review comments, "
                "a calibrated self-reported confidence between 0 and 1, and a short "
                "confidence_reasoning. If confidence is low, populate escalation_questions "
                "with specific clarifying questions."
            )},
            {"role": "user", "content": (
