...
def node_analyze(state):
    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                "You are a senior code reviewer. Produce a structured PRAnalysis. "
                "Calibrate confidence honestly: high (≥0.73) only when the change is "
                "mechanical and you are confident nothing is missed; medium (0.58–0.72) "
                "for normal features that benefit from a human check; low (<0.58) when "
                "the diff touches auth/security/migrations or you have specific doubts. "
                "If your confidence is below 0.60, you MUST populate escalation_questions "
                "with 2–4 SPECIFIC, context-rich questions for a human reviewer. "
                "Each question must reference the concrete file and section in the diff "
                "(e.g. 'In auth.py, why is MD5 used for password hashing?'). "
                "Avoid generic questions like 'is this safe?' — ask about exact code."
            )},
...
def node_escalate(state: ReviewState) -> dict:
    ...
    console.print(f"[red]→ escalate[/red]  asking {len(questions)} question(s)")
    answers = interrupt({
        "kind": "escalation",
        "pr_url": state["pr_url"],
...
def node_synthesize(state: ReviewState) -> dict:
    ...
    qa = "\n".join(
        f"Q: {q}\nA: {ans}"
        for q, ans in (state.get("escalation_answers") or {}).items()
    )
    llm = get_llm().with_structured_output(PRAnalysis)
    with console.status("[dim]LLM refining review with reviewer answers...[/dim]"):
        refined: PRAnalysis = llm.invoke([
            {"role": "system", "content": (
                "You are a senior reviewer producing a REFINED structured review. "
                "You previously had low confidence and asked the reviewer questions; "
                "their answers are below. Update your summary, risk_factors, comments, "
                "and confidence in light of the answers — confidence should usually rise. "
                "Do not invent issues that the reviewer's answers resolve."
            )},
            {"role": "user", "content": (
