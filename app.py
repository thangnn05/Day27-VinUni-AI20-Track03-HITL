...
async def fetch_recent_sessions(limit: int = 15) -> list[dict]:
...
with st.sidebar:
    ...
    try:
        sessions = asyncio.run(fetch_recent_sessions())
    except Exception:
        sessions = []
    if not sessions:
    ...
    for s in sessions:
        short_tid = s["thread_id"][:8]
        emoji = risk_emoji.get(s["worst_risk"], "⚪")
        label = f"{emoji} {short_tid} · {s['pr_url'].rsplit('/', 1)[-1]}"
        if st.button(label, key=f"sess_{s['thread_id']}", use_container_width=True):
            st.session_state.thread_id = s["thread_id"]
            st.session_state.pr_url = s["pr_url"]
            st.session_state.interrupt_payload = None
            st.session_state.final = None
            st.rerun()
        st.caption(f"{s['events']} events · last {s['last_event'][:19]}")
...
def render_escalation_card(payload: dict) -> dict | None:
    ...
    with st.form("escalation"):
        ...
        for i, q in enumerate(payload.get("questions", [])):
            answers[q] = st.text_area(q, key=f"esc_q_{i}", height=80)
